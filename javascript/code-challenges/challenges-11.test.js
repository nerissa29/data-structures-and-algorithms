...
// used flat(0 and map() from MDN
const count = (target, input) => {
  // uses flat() to flatten nested arrays, and then uses map() to iterate through the array values
  let targetCount = 0;
  input.flat().map(element => {
    if (element === target) targetCount++;
  });
  return targetCount;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((accu, curVal) => accu + curVal, 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  // let person = [];
  let names = data.filter(element => element.gender === 'female' || element.gender === 'male');
  return names.map(value => value.name).join(' and ');
};
